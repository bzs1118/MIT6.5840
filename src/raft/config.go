...
func (cfg *config) checkLogs(i int, m ApplyMsg) (string, bool) {
	...
	//DPrintf("len(server %v logs) = %v", i, len(cfg.logs[i]))
	...
	for m := range applyCh {
		...
		if m.SnapshotValid {
			...
			if (m.CommandIndex+1)%SnapShotInterval == 0 {
				DPrintf("m.CommandIndex: %v", m.CommandIndex)
				w := new(bytes.Buffer)
				...
				e.Encode(xlog)
				go rf.Snapshot(m.CommandIndex, w.Bytes())
			}
		} else {
			// Ignore other types of ApplyMsg.
		}
...
func (cfg *config) start1(i int, applier func(int, chan ApplyMsg)) {
	...
	cfg.mu.Lock()
	cfg.rafts[i] = rf
	cfg.lastApplied[i] = rf.lastIncludedIndex + 1 // I set initial lastApplied is lastIncludedIndex + 1; original version is 0.
	cfg.mu.Unlock()
